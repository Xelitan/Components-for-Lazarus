...
// this encoder just wrote with h265_hevc_ps's parsers, so every derived
// field is what the decoder will compute;
// * prediction is h265_hevcpred.intra_pred, not a reimplementation;
...
uses
  Math,
  h265_common, h265_bits, h265_putbits, h265_hevc_defs, h265_frame, h265_hevc_ps,
  h265_hevc, h265_hevcpred, h265_hevcdsp, h265_hevcdsp_enc, h265_hevc_cabac,
  h265_hevc_refs, h265_hevc_mvs, h265_cabac_enc, h265_syntax_enc, h265_ps_enc,
  h265_slice_enc, h265_sao_enc;
...
// coding block size (8).
function h265_enc_init(var Enc: TH265Encoder;
...
function h265_enc_picture(var Enc: TH265Encoder): Integer;
procedure h265_enc_free(var Enc: TH265Encoder);
...
type
  TEncodeCuProc = procedure(var Enc: TH265Encoder; X0, Y0, Log2CbSize: Integer);
var
  enc_cu_hook: TEncodeCuProc = nil;

procedure encode_coding_unit(var Enc: TH265Encoder; X0, Y0, Log2CbSize: Integer);
...
procedure encode_coding_unit_part(var Enc: TH265Encoder;
  X0, Y0, Log2CbSize, ForcePart: Integer);

// Decides PART_NxN against PART_2Nx2N at the minimum coding block size;
// h265_enc_rd installs a decider. Nil means always PART_2Nx2N.
type
  TCuPartFunc = function(var Enc: TH265Encoder;
    X0, Y0, Log2CbSize: Integer): Boolean;
var
  enc_cu_part_hook: TCuPartFunc = nil;

// Decides split_transform_flag for one transform tree node; h265_enc_rd
// installs a rate-distortion decider. Nil means never split.
type
  TTuSplitFunc = function(var Enc: TH265Encoder;
...
procedure encode_transform_node(var Enc: TH265Encoder;
  X0, Y0, Log2CbSize, Log2TrafoSize, TrafoDepth, ForceSplit: Integer);

// Replaces the whole quadtree walk of one CTU; h265_enc_rd installs the
// rate-distortion version here. Returns "more data", like encode_quadtree.
type
  TEncodeQuadtreeFunc = function(var Enc: TH265Encoder;
...
// h265_enc_init, so set it before initialising.
procedure h265_enc_lossless(On_: Boolean);
function h265_enc_is_lossless: Boolean;
...
// see task #28.
procedure h265_enc_rdoq_full(On_: Boolean);
...
procedure h265_enc_screen(On_: Boolean);
...
procedure h265_enc_sao(On_: Boolean);
...
procedure h265_enc_deblock(On_: Boolean);
...
procedure h265_enc_modes(On_: Boolean);

// Cross-component prediction (4:4:4 only).
procedure h265_enc_ccp(On_: Boolean);

// Where the RDOQ loop measures its distortion.
procedure h265_enc_rdoq_tdomain(On_: Boolean);

// the end_of_slice_flag bookkeeping a quadtree leaf performs
function encode_leaf_tail(var Enc: TH265Encoder; X0, Y0, cb_size: Integer): Integer;
...
function rdoq_trim_last(var Enc: TH265Encoder; Coeffs, PreQ: PInt16;
...
function encode_tb(var Enc: TH265Encoder; X0, Y0, Log2Size, CIdx: Integer;
...
procedure reconstruct_tb(var Enc: TH265Encoder; X0, Y0, Log2Size, CIdx: Integer;
...
function chroma_mode_cost(var Enc: TH265Encoder; X0, Y0, Log2Size, Mode: Integer;
...
function choose_luma_mode(var Enc: TH265Encoder; X0, Y0, Log2Size: Integer): Integer;
var
  S: PHEVCContext;
  LC: PHEVCLocalContext;
...
begin
  ...
  // lambda needs the same mean-weight rescale h265_enc_rd applies -- otherwise
...
procedure encode_tt_leaf(var Enc: TH265Encoder;
...
procedure encode_tt_split_min(var Enc: TH265Encoder;
...
procedure encode_cu_nxn(var Enc: TH265Encoder; X0, Y0, Log2CbSize: Integer);
var
  S: PHEVCContext;
  LC: PHEVCLocalContext;
...
procedure encode_cu_2nx2n(var Enc: TH265Encoder; X0, Y0, Log2CbSize: Integer);
var
  S: PHEVCContext;
  LC: PHEVCLocalContext;
...
function encode_quadtree(var Enc: TH265Encoder;
  X0, Y0, Log2CbSize, CbDepth: Integer): Integer;
var
  S: PHEVCContext;
