...
// Corresponds to: libbpg.c from clamp_pix() to h265_decoder_get_line().
//
...
uses
  Math, h265_common, h265_container;

function h265_decoder_start(S: PH265DecoderContext; out_fmt: Integer): Integer;
function h265_decoder_get_line(S: PH265DecoderContext; rgb_line1: Pointer): Integer;
procedure h265_decoder_output_end(S: PH265DecoderContext);
...
type
  // the vertical interpolation ring buffer, as held by TH265DecoderContext
...
const
  cs_to_rgb24: array[0 .. H265_CS_COUNT - 1] of TColorConvertFunc = (
    ycc_to_rgb24, rgb_to_rgb24, ycgco_to_rgb24, ycc_to_rgb24, ycc_to_rgb24);
  cs_to_rgb48: array[0 .. H265_CS_COUNT - 1] of TColorConvertFunc = (
...
begin
  ...
  is_ycc := True;
  case color_space of
    H265_CS_YCbCr:        begin k_r := 0.299;  k_b := 0.114;  end;
    H265_CS_YCbCr_BT709:  begin k_r := 0.2126; k_b := 0.0722; end;
    H265_CS_YCbCr_BT2020: begin k_r := 0.2627; k_b := 0.0593; end;
  else
    is_ycc := False;
  end;
  if is_ycc then
...
function h265_decoder_output_init(S: PH265DecoderContext; out_fmt: Integer): Integer;
var
  I, out_bd: Integer;
begin
  if (out_fmt < 0) or (out_fmt > H265_OUTPUT_FORMAT_CMYK64) then Exit(-1);
  S^.is_rgba := Byte(Ord((out_fmt = H265_OUTPUT_FORMAT_RGBA32) or
                         (out_fmt = H265_OUTPUT_FORMAT_RGBA64)));
  S^.is_16bpp := Byte(Ord((out_fmt = H265_OUTPUT_FORMAT_RGB48) or
                          (out_fmt = H265_OUTPUT_FORMAT_RGBA64) or
                          (out_fmt = H265_OUTPUT_FORMAT_CMYK64)));
  S^.is_cmyk := Byte(Ord((out_fmt = H265_OUTPUT_FORMAT_CMYK32) or
                         (out_fmt = H265_OUTPUT_FORMAT_CMYK64)));

  if (S^.format = H265_FORMAT_420) or (S^.format = H265_FORMAT_422) then
  begin
    S^.w2 := (S^.w + 1) div 2;
    S^.h2 := (S^.h + 1) div 2;
    ...
    if S^.format = H265_FORMAT_420 then
  ...
  if S^.format = H265_FORMAT_GRAY then
  begin
...
begin
  ...
  begin
    ...
    Ret := h265_decoder_output_init(S, out_fmt);
  ...
  begin
    ...
    if h265_decoder_decode_next_frame(S) < 0 then Exit(-1);
  end
  else
    Exit(-1);

  S^.y_buf := h265_decoder_get_data(S, S^.y_linesize, 0);
  if S^.format <> H265_FORMAT_GRAY then
  begin
    S^.cb_buf := h265_decoder_get_data(S, S^.cb_linesize, 1);
    S^.cr_buf := h265_decoder_get_data(S, S^.cr_linesize, 2);
    c_idx := 3;
  end
  else
    c_idx := 1;
  if S^.has_alpha <> 0 then
    S^.a_buf := h265_decoder_get_data(S, S^.a_linesize, c_idx)
  else
    S^.a_buf := nil;
  S^.y := 0;
  Result := 0;
