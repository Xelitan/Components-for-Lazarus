...
// Corresponds to: the first half of libbpg.c (up to h265_decoder_get_info) and
// h265_decode_header / h265_decoder_decode near its end.
//
// The reference talks to libavcodec through AVCodecContext; here the two HEVC
// decoders are plain THEVCContext records driven by h265_hevc directly.
//
// Colour conversion and line output live in h265_output.
unit h265_container;
...
uses
  h265_common, h265_hevc_defs, h265_frame, h265_hevc;

const
  H265_HEADER_MAGIC = $425047FB;
  MAX_DATA_SIZE    = (1 shl 30) - 1;

  // H265ImageFormatEnum
  H265_FORMAT_GRAY       = 0;
  H265_FORMAT_420        = 1; // chroma at offset (0.5, 0.5) (JPEG)
  H265_FORMAT_422        = 2; // chroma at offset (0.5, 0)   (JPEG)
  H265_FORMAT_444        = 3;
  H265_FORMAT_420_VIDEO  = 4; // chroma at offset (0, 0.5)   (MPEG2)
  H265_FORMAT_422_VIDEO  = 5; // chroma at offset (0, 0)     (MPEG2)

  // H265ColorSpaceEnum
  H265_CS_YCbCr          = 0;
  H265_CS_RGB            = 1;
  H265_CS_YCgCo          = 2;
  H265_CS_YCbCr_BT709    = 3;
  H265_CS_YCbCr_BT2020   = 4;
  H265_CS_COUNT          = 5;

  // H265ExtensionTagEnum
  H265_EXTENSION_TAG_EXIF         = 1;
  H265_EXTENSION_TAG_ICCP         = 2;
  H265_EXTENSION_TAG_XMP          = 3;
  H265_EXTENSION_TAG_THUMBNAIL    = 4;
  H265_EXTENSION_TAG_ANIM_CONTROL = 5;

  // H265DecoderOutputFormat
  H265_OUTPUT_FORMAT_RGB24  = 0;
  H265_OUTPUT_FORMAT_RGBA32 = 1;
  H265_OUTPUT_FORMAT_RGB48  = 2;
  H265_OUTPUT_FORMAT_RGBA64 = 3;
  H265_OUTPUT_FORMAT_CMYK32 = 4;
  H265_OUTPUT_FORMAT_CMYK64 = 5;

  ITAPS2 = 4;
  ITAPS  = 2 * ITAPS2;

type
  PH265ExtensionData = ^TH265ExtensionData;
  TH265ExtensionData = record
    tag: Cardinal;
    buf_len: Cardinal;
    buf: PByte;
    next: PH265ExtensionData;
  end;

  TH265ImageInfo = record
    width: Cardinal;
    height: Cardinal;
    format: Byte;
    has_alpha: Byte;
  ...
  PH265DecoderContext = ^TH265DecoderContext;

  // Signature of the per-line colour converters in h265_output; kept here so the
  ...
  TH265DecoderContext = record
    ...
    // format conversion (driven by h265_output)
    output_inited: Byte;
    out_fmt: Integer;
    is_rgba: Byte;
    is_16bpp: Byte;
  ...
  PH265HeaderData = ^TH265HeaderData;

function h265_decoder_open: PH265DecoderContext;
function h265_decoder_decode(Img: PH265DecoderContext; Buf: PByte; buf_len: Integer): Integer;
procedure h265_decoder_close(S: PH265DecoderContext);
function h265_decoder_get_data(Img: PH265DecoderContext; out line_size: Integer;
  Plane: Integer): PByte;
function h265_decoder_get_info(Img: PH265DecoderContext; P: PH265ImageInfo): Integer;
procedure h265_decoder_free_extension_data(first_md: PH265ExtensionData);
procedure h265_decoder_keep_extension_data(S: PH265DecoderContext; Enable: Integer);
function h265_decoder_get_extension_data(S: PH265DecoderContext): PH265ExtensionData;
procedure h265_decoder_get_frame_duration(S: PH265DecoderContext; out Num, Den: Integer);
function h265_decoder_get_frame_count(S: PH265DecoderContext): Integer;

// animation: decode the next frame; 0 on success, < 0 when the stream ends
function h265_decoder_decode_next_frame(S: PH265DecoderContext): Integer;

function h265_decode_header(H: PH265HeaderData; Buf: PByte; buf_len: Integer;
...
function hevc_decode_frame_internal(S: PH265DecoderContext;
...
function hevc_decode_start(S: PH265DecoderContext; Buf: PByte; buf_len1: Integer;
...
function hevc_decode_next(S: PH265DecoderContext; Buf: PByte; buf_len: Integer): Integer;
var
  abuf_s, cbuf_s: TDynBuf;
  abuf, cbuf: PDynBuf;
begin
...
procedure hevc_decode_end(S: PH265DecoderContext);
begin
  if S^.alpha_dec_ctx <> nil then
  begin
...
var
  c_count: Integer;
begin
  if Img^.format = H265_FORMAT_GRAY then c_count := 1 else c_count := 3;
...
var
  ...
  md: PH265ExtensionData;
  plast_md: ^PH265ExtensionData;
  loop_count, frame_delay_num, frame_delay_den: Cardinal;
label
  fail;
begin
  if buf_len < 6 then Exit(-1);
  if (Buf[0] <> ((H265_HEADER_MAGIC shr 24) and $FF)) or
     (Buf[1] <> ((H265_HEADER_MAGIC shr 16) and $FF)) or
     (Buf[2] <> ((H265_HEADER_MAGIC shr 8) and $FF)) or
     (Buf[3] <> (H265_HEADER_MAGIC and $FF)) then Exit(-1);
  ...
  if (H^.color_space >= H265_CS_COUNT) or
     ((H^.format = H265_FORMAT_GRAY) and (H^.color_space <> 0)) or
     ((H^.has_w_plane <> 0) and (H^.format = H265_FORMAT_GRAY)) then Exit(-1);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (H^.has_animation <> 0) and (Tag = H265_EXTENSION_TAG_ANIM_CONTROL) then
        begin
          idx1 := Idx;
        ...
        begin
          md := av_malloc(SizeOf(TH265ExtensionData));
  ...
  Exit(Idx);
fail:
  h265_decoder_free_extension_data(H^.first_md);
  H^.first_md := nil;
  Result := -1;
end;
...
function h265_decoder_open: PH265DecoderContext;
begin
  Result := av_mallocz(SizeOf(TH265DecoderContext));
end;
...
label
  fail;
begin
  Idx := h265_decode_header(@H, Buf, buf_len, 0, Img^.keep_extension_data);
  ...
  if H.format = H265_FORMAT_422_VIDEO then
  begin
    Img^.format := H265_FORMAT_422;
    Img^.c_h_phase := 0;
  end
  else if H.format = H265_FORMAT_420_VIDEO then
  begin
    Img^.format := H265_FORMAT_420;
    Img^.c_h_phase := 0;
  end
  else
  begin
...
fail:
  ...
  h265_decoder_free_extension_data(Img^.first_md);
  Img^.first_md := nil;
  Result := -1;
end;
...
procedure h265_decoder_close(S: PH265DecoderContext);
begin
  // h265_output.h265_decoder_output_end must have been called by the caller
  av_free(S^.input_buf);
  hevc_decode_end(S);
  av_frame_free(S^.frame);
  av_frame_free(S^.alpha_frame);
  h265_decoder_free_extension_data(S^.first_md);
  av_free(S);
end;
